...
Credential precedence: when `providers.meta.access_token` is present inline in the CLI metadata file `~/.config/muse/auth.json`, that token selects the account queried and takes precedence over Keychain. Otherwise CodexBar reads the device-code token from the CLI's Keychain item. An `auth.json` with `"mechanism": "oauth"` but no inline token still counts as a login; the token then comes from Keychain. Override the file path with `MUSE_AUTH_PATH` if needed. Scheduled, menu-open, and CLI refreshes never prompt Keychain: every refresh first preflights the item's decrypt ACL and only requests secret bytes when access is already granted, otherwise failing closed. An explicit manual Refresh (global or the Muse card's refresh) may show one authorization prompt when access is not granted yet; choose Always Allow and later refreshes read silently. A grant is bound to the exact app build, so a rebuilt development copy needs authorization again, as does an item recreated by the CLI. Resolved tokens are cached in memory for the app session, so the CLI's own credential rewrites (which reset the item ACL) do not break later refreshes; a rejected token clears the cache and retries once against a re-read credential.
