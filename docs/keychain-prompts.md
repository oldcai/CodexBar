...
- Muse Code subscription reads the CLI-owned `ai.meta.dev.credentials` item only after a no-UI preflight proves
  access is already granted; otherwise scheduled refreshes fail closed without prompting, while an explicit manual
  Refresh may ask once so you can authorize access. See [Muse Code](muse.md).
